...
func (uc *fakeWorkOrderUsecase) GetByOnchainOrderID(ctx context.Context, onchainOrderID string) (*domain.WorkOrder, error) {
	uc.getCalls++
	uc.getOrderID = onchainOrderID
	if uc.getErr != nil {
		return nil, uc.getErr
	}

	return uc.getOutput, nil
}
...
func serveGetWorkOrder(handler *WorkOrderHandler, rec *httptest.ResponseRecorder, req *nethttp.Request) {
	router := chi.NewRouter()
	router.Get("/api/work-orders/{onchainOrderID}", handler.Get)
	router.ServeHTTP(rec, req)
}

func TestWorkOrderHandlerGetSuccess(t *testing.T) {
	deliverable := "0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
	usecase := &fakeWorkOrderUsecase{
		getOutput: &domain.WorkOrder{
			ID:             uuid.MustParse("018f95e4-3f8d-7b70-a4dd-2d9a833c4a1f"),
	...
	handler := NewWorkOrderHandler(usecase)
	req := httptest.NewRequest(nethttp.MethodGet, "/api/work-orders/123", nil)
	rec := httptest.NewRecorder()

	serveGetWorkOrder(handler, rec, req)

	if rec.Code != nethttp.StatusOK {
		t.Fatalf("expected status 200, got %d: %s", rec.Code, rec.Body.String())
	}
	if usecase.getOrderID != "123" {
		t.Fatalf("expected path order id 123, got %q", usecase.getOrderID)
	}
	...
	if body.Data.Status != string(domain.WorkOrderStatusFunded) {
		t.Fatalf("expected status funded, got %q", body.Data.Status)
	}
	if body.Data.DeliverableCID == nil || *body.Data.DeliverableCID != deliverable {
		t.Fatalf("expected deliverable_cid %q, got %v", deliverable, body.Data.DeliverableCID)
	}
}

func TestWorkOrderHandlerGetErrorMapping(t *testing.T) {
	tests := []struct {
		name       string
		err        error
		statusCode int
		...
		{name: "validation", err: domain.NewValidationError("onchainOrderID must be a positive integer"), statusCode: nethttp.StatusBadRequest, code: "BAD_REQUEST"},
		{name: "not found", err: domain.ErrNotFound, statusCode: nethttp.StatusNotFound, code: "WORK_ORDER_NOT_FOUND"},
		{name: "persistence", err: errors.Join(domain.ErrPersistence, errors.New("query failed")), statusCode: nethttp.StatusInternalServerError, code: "FAILED_TO_FETCH_WORK_ORDER"},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			handler := NewWorkOrderHandler(&fakeWorkOrderUsecase{getErr: tt.err})
			req := httptest.NewRequest(nethttp.MethodGet, "/api/work-orders/123", nil)
			rec := httptest.NewRecorder()
